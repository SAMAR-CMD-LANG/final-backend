...
class EnhancedHabitService {
    /**
     * Check if enhanced columns exist
     */
    async hasEnhancedColumns() {
        try {
    ...
    /**
     * Create a new habit with enhanced features
     */
    ...
    /**
     * Get all habits for a user with filtering and sorting
     */
    async getUserHabits(userId, options = {}) {
        ...
        // Apply enhanced filters only if columns exist
        if (hasEnhanced) {
            if (filter.category) {
    ...
    /**
     * Get all categories used by a user (enhanced feature)
     */
    ...
    /**
     * Archive/unarchive a habit (enhanced feature)
     */
    async toggleArchiveHabit(habitId, userId, isArchived) {
        const hasEnhanced = await this.hasEnhancedColumns();
        if (!hasEnhanced) {
            throw new Error('Archive functionality requires database migration');
        }
    ...
    async updateHabit(habitId, userId, updateData) {
        const hasEnhanced = await this.hasEnhancedColumns();
        const allowedFields = hasEnhanced
            ? ['title', 'description', 'category', 'is_archived']
            : ['title', 'description'];
...
module.exports = new EnhancedHabitService();